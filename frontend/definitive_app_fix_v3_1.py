...
# These were { /* comment */ } but should be // comment outside of JSX.
...
def restore_nonjsx_comments(text):
    ...
    for line in lines:
        # Check for start of App component (using the found pattern)
        if 'function App() {' in line:
            in_component_setup = True
        # Check for start of JSX return block
        if 'return (' in line:
            in_component_setup = False
...
print("Definitive App.jsx restoration v3.1 complete.")
